...
use helix_stdx::path as hx_path;
...
impl Explorer {
    ...
    pub fn from_path(root: PathBuf, column_width: u16) -> Result<Self> {
        Ok(Self {
    ...
    fn new_file(&mut self, path: &str) -> Result<()> {
        let path = hx_path::normalize(PathBuf::from(path));
    ...
    for id in ids {
        cx.editor
            ...
            .map_err(|e| match e {
                ...
                helix_view::editor::CloseError::SaveError(err) => err,
            })?;
    }
    Ok(())
}
